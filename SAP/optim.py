...
def main():
    ...
    parser.add_argument(
        "--seed", type=int, default=1457, metavar="S", help="random seed"
    )
    ...
    print(cfg["training"]["out_dir"])

    # PYTORCH VERSION > 1.0.0
    assert float(torch.__version__.split(".")[-3]) > 0

    # boiler-plate
    if cfg["training"]["timestamp"]:
        cfg["training"]["out_dir"] += "_" + time.strftime("%Y_%m_%d_%H_%M_%S")
    logger = initialize_logger(cfg)
    torch.manual_seed(args.seed)
    torch.cuda.manual_seed_all(args.seed)
    np.random.seed(args.seed)
    shutil.copyfile(
        args.config, os.path.join(cfg["training"]["out_dir"], "config.yaml")
    )

    # tensorboardX writer
    tblogdir = os.path.join(cfg["training"]["out_dir"], "tensorboard_log")
    ...
    vis = None
    if cfg["training"]["o3d_show"]:
        vis = o3d.visualization.Visualizer()
        vis.create_window(
            width=cfg["training"]["o3d_window_size"],
            height=cfg["training"]["o3d_window_size"],
        )
    ...
    if data_type == "point":
        if cfg["data"]["object_id"] != -1:
            ...
            print(
                "Loaded %d/%d object" % (cfg["data"]["object_id"] + 1, len(data_paths))
            )
        else:
        ...
        if ext == "obj":  # have GT mesh
            ...
            target_pts, target_normals = sample_points_from_meshes(
                mesh, num_samples=200000, return_normals=True
            )
    ...
    if "target_points" in data.keys():
        outdir_pcl = os.path.join(cfg["training"]["out_dir"], "target_pcl.ply")
    ...
    if data.get("gt_mesh") is not None:
        ...
        pts_gt, norms_gt = sample_points_from_meshes(
            data["gt_mesh"], num_samples=500000, return_normals=True
        )
        pts_gt = (pts_gt + 1) / 2
        ...
        outdir_mesh = os.path.join(cfg["training"]["out_dir"], "oracle_mesh.ply")
        o3d.io.write_triangle_mesh(outdir_mesh, mesh)

    # initialize the source point cloud given an input mesh
    if "input_mesh" in cfg["training"].keys() and os.path.isfile(
        cfg["training"]["input_mesh"]
    ):
        if cfg["training"]["input_mesh"].split("/")[-2] == "mesh":
            mesh_tmp = trimesh.load_mesh(cfg["training"]["input_mesh"])
            ...
            pcd = o3d.io.read_point_cloud(cfg["training"]["input_mesh"])
    ...
    cfg["training"]["schedule"]["pcl"]["initial"] = cfg["training"]["lr_pcl"]
    print("Initial learning rate:", cfg["training"]["schedule"]["pcl"]["initial"])
    if "schedule" in cfg["training"]:
        lr_schedules = get_learning_rate_schedules(cfg["training"]["schedule"])
    else:
        lr_schedules = None

    optimizer = update_optimizer(
        inputs, cfg, epoch=0, model=model, schedule=lr_schedules
    )

    try:
        # load model
        state_dict = torch.load(os.path.join(cfg["training"]["out_dir"], "model.pt"))
        if ("pcl" in state_dict.keys()) & (state_dict["pcl"] is not None):
            inputs = state_dict["pcl"].to(device)
            inputs.requires_grad = True

        optimizer = update_optimizer(
            inputs, cfg, epoch=state_dict.get("epoch"), schedule=lr_schedules
        )
    ...
    # training loop
    for epoch in range(start_epoch + 1, cfg["training"]["total_epochs"] + 1):
        ...
        if (epoch > 0) & (lr_schedules is not None):
            if epoch % lr_schedules[0].interval == 0:
                ...
                if len(lr_schedules) > 1:
                    ...
                    print(
                        "[epoch {}] adjust pcl_lr to: {}".format(
                            epoch, lr_schedules[0].get_learning_rate(epoch)
                        )
                    )
        ...
        if epoch % cfg["training"]["print_every"] == 0:
            ...
            log_text += (" time=%.3f / %.3f") % (runtime["all"].val, runtime["all"].sum)
            logger.info(log_text)
            print(log_text)

        # visualize point clouds and meshes
        if (epoch % cfg["training"]["visualize_every"] == 0) & (vis is not None):
            trainer.visualize(data, inputs, model, epoch, o3d_vis=vis)

        # save outputs
        if epoch % cfg["training"]["save_every"] == 0:
            trainer.save_mesh_pointclouds(
                inputs, epoch, center.cpu().numpy(), scale.cpu().numpy() * (1 / 0.9)
            )

        # save checkpoints
        if (epoch > 0) & (epoch % cfg["training"]["checkpoint_every"] == 0):
            state = {"epoch": epoch}
            pcl = None
            ...
            torch.save(
                state,
                os.path.join(cfg["training"]["dir_model"], "%04d" % epoch + ".pt"),
            )
            print("Save new model at epoch %d" % epoch)
            logger.info("Save new model at epoch %d" % epoch)
            torch.save(state, os.path.join(cfg["training"]["out_dir"], "model.pt"))

        # resample and gradually add new points to the source pcl
        if (
            (epoch > 0)
            & (cfg["training"]["resample_every"] != 0)
            & (epoch % cfg["training"]["resample_every"] == 0)
            & (epoch < cfg["training"]["total_epochs"])
        ):
            inputs = trainer.point_resampling(inputs)
            optimizer = update_optimizer(
                inputs, cfg, epoch=epoch, model=model, schedule=lr_schedules
            )
            trainer = Trainer(cfg, optimizer, device=device)

    # visualize the Open3D outputs
    if cfg["training"]["o3d_show"]:
        out_video_dir = os.path.join(cfg["training"]["out_dir"], "vis/o3d/video.mp4")
        ...
        os.system(
            "ffmpeg -framerate 30 \
                    ...
                    -pix_fmt yuv420p \
                    -crf 17 {}".format(
                cfg["training"]["out_dir"], out_video_dir
            )
        )
        out_video_dir = os.path.join(
            cfg["training"]["out_dir"], "vis/o3d/video_pcd.mp4"
        )
        if os.path.isfile(out_video_dir):
            os.system("rm {}".format(out_video_dir))
        os.system(
