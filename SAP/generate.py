...
def main():
    ...
    parser.add_argument(
        "--iter", type=int, metavar="S", help="the training iteration to be evaluated."
    )
    ...
    try:
        if is_url(cfg["test"]["model_file"]):
            ...
            state_dict = torch.load(
                os.path.join(out_dir, "model", "%04d.pt" % cfg["generation"]["iter"])
            )
            generation_dir += "_%04d" % cfg["generation"]["iter"]
        elif args.iter is not None:
            state_dict = torch.load(
                os.path.join(out_dir, "model", "%04d.pt" % args.iter)
            )
        else:
    ...
    for it, data in enumerate(tqdm(test_loader)):
        ...
        if generate_mesh:
            ...
            mesh_out_file = os.path.join(
                mesh_dir, "%s.ply" % modelname
            )  # change to ply
            export_mesh(mesh_out_file, v, f)  # change not scale scale2onet(v) ->v
            out_file_dict["mesh"] = mesh_out_file

        if generate_pointcloud:
            pointcloud_out_file = os.path.join(pointcloud_dir, "%s.ply" % modelname)
            export_pointcloud(pointcloud_out_file, points, normals)  # change not sacle
            out_file_dict["pointcloud"] = pointcloud_out_file

        if cfg["generation"]["copy_input"]:
            inputs_path = os.path.join(in_dir, "%s.ply" % modelname)
            p = data.get("inputs").to(device)
            export_pointcloud(inputs_path, p)  # change not scale
        ...
        if c_it < vis_n_outputs:
            ...
            if cfg["generation"]["exp_oracle"]:
                ...
                out_file = os.path.join(
                    generation_vis_dir, "%02d_%s%s" % (c_it, "mesh_oracle", ".off")
                )
                export_mesh(out_file, v, f)  # change not scale
