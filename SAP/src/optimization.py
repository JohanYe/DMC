...
class Trainer(object):
    ...
    def compute_loss(self, inputs, data, model, it=0):
        ...
        if self.data_type == "point":
            if self.cfg["training"]["w_chamfer"] > 0:
                loss_ = self.cfg["training"]["w_chamfer"] * self.compute_3d_loss(
                    v, data
                )
    ...
    def compute_3d_loss(self, v, data):
        ...
        idx = np.random.randint(
            pts_gt.shape[1], size=self.cfg["training"]["n_sup_point"]
        )
        if self.cfg["training"]["subsample_vertex"]:
            # chamfer distance only on random sampled vertices
            idx = np.random.randint(
                v.shape[1], size=self.cfg["training"]["n_sup_point"]
            )
    ...
    def compute_2d_loss(self, inputs, data, model):
        ...
        losses = {
            "color": {"weight": self.cfg["training"]["l_weight"]["rgb"], "values": []},
            "silhouette": {
                "weight": self.cfg["training"]["l_weight"]["mask"],
                "values": [],
            },
        }
        ...
        if out["rgb"] is not None:
            rgb_gt = out["rgb_gt"].reshape(self.cfg["data"]["n_views_per_iter"], -1, 3)[
                out["vis_mask"]
            ]
            loss_all["color"] += (
                torch.nn.L1Loss(reduction="sum")(rgb_gt, out["rgb"])
                / out["rgb"].shape[0]
            )
    ...
    def point_resampling(self, inputs):
        ...
        # shortcuts
        n_grow = self.cfg["training"]["n_grow_points"]
    ...
    def visualize(self, data, inputs, renderer, epoch, o3d_vis=None):
        ...
        it = "{:04d}".format(int(epoch / self.cfg["training"]["visualize_every"]))

        if (
            (self.cfg["training"]["exp_mesh"])
            | (self.cfg["training"]["exp_pcl"])
            | (self.cfg["training"]["o3d_show"])
        ):
            psr_grid, points, normals = self.pcl2psr(inputs)
            ...
            color_v = None
            if data_type == "img":
                if self.cfg["training"]["vis_vert_color"] & (
                    self.cfg["training"]["l_weight"]["rgb"] != 0.0
                ):
            ...
            points = points * 2 - 1
            visualize_points_mesh(
                o3d_vis, points, normals, vv, ff, self.cfg, it, epoch, color_v=color_v
            )

        else:
            v, f, n = inputs

        if (data_type == "img") & (self.cfg["training"]["vis_rendering"]):
            pred_imgs = []
            pred_masks = []
            ...
            for idx in idx_list:
                ...
                img_size = (
                    ...
                    else (rgb.shape[0], rgb.shape[1])
                )
                ray = None
                if "rays" in data.keys():
                    ray = data["rays"][idx]
                if self.cfg["training"]["l_weight"]["rgb"] != 0.0:
                    fea_grid = None
                    ...
                    if model.encoder is not None:
                        ...
                        fea_grid = model.encoder(pp, normalize=False).permute(
                            0, 2, 3, 4, 1
                        )
                    ...
                    filename = os.path.join(
                        self.cfg["training"]["dir_rendering"],
            ...
            if len(pred_imgs) >= 1:
                ...
                save_image(
                    pred_imgs.permute(0, 3, 1, 2),
                    os.path.join(
                        self.cfg["training"]["dir_rendering"], "{}.png".format(it)
                    ),
                    nrow=4,
                )
                if self.cfg["training"]["save_video"]:
                    write_video(
                        os.path.join(
                            self.cfg["training"]["dir_rendering"], "{}.mp4".format(it)
                        ),
    ...
    def save_mesh_pointclouds(self, inputs, epoch, center=None, scale=None):
        ...
        exp_pcl = self.cfg["training"]["exp_pcl"]
        exp_mesh = self.cfg["training"]["exp_mesh"]

        psr_grid, points, normals = self.pcl2psr(inputs)

        if exp_pcl:
            dir_pcl = self.cfg["training"]["dir_pcl"]
        ...
        if exp_mesh:
            dir_mesh = self.cfg["training"]["dir_mesh"]
            with torch.no_grad():
                v, f, _ = mc_from_psr(
                    psr_grid, zero_level=self.cfg["data"]["zero_level"], real_scale=True
                )
                v = v * 2 - 1
                if scale is not None:
                    v *= scale
        ...
        if self.cfg["training"]["vis_psr"]:
            dir_psr_vis = self.cfg["training"]["out_dir"] + "/psr_vis_all"
