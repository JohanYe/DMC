...
class SAP2Image(nn.Module):
    def __init__(self, cfg, img_size):
        ...
        self.cfg = cfg
        if cfg["training"]["l_weight"]["rgb"] != 0.0:
            self.rendering_network = RenderingNetwork(**cfg["model"]["renderer"])

        if cfg["training"]["l_weight"]["mask"] != 0.0:
    ...
    def render_img(self, psr_grid, data):
        ...
        if self.cfg["training"]["l_weight"]["rgb"] != 0.0:
            ...
            p_inters, visible_mask = self.psr2sur(
                psr_grid, pose, self.img_size, uv, psr_grad, None
            )
            ...
            pred_rgb = self.rendering_network(
                p_inters,
                normals=n_inters.squeeze(),
                view_dirs=ray,
                feature_vectors=fea_interp,
            )

        # silhouette loss
        if self.cfg["training"]["l_weight"]["mask"] != 0.0:
            # build mesh
            ...
            if True:
                ...
                pose_cur = PerspectiveCameras(
                    focal_length=focal, principal_point=pp, R=R, T=T, device=R.device
                )
                pred_mask = self.mesh_rasterizer(
                    mesh.extend(n_views_per_iter), cameras=pose_cur
                )[..., 3]
            else:
                pred_mask = []
                ...
                for i in range(n_views_per_iter):
                    #! PyTorch3D silhouette loss
                    pred_mask.append(
                        self.mesh_rasterizer(mesh, cameras=pose[i])[..., 3]
                    )
