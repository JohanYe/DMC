...
def mc_from_psr(psr_grid, pytorchify=False, real_scale=False, zero_level=0):
    ...
    if batch_size > 1:
        ...
        for i in range(batch_size):
            verts_cur, faces_cur, normals_cur, values = measure.marching_cubes(
                psr_grid_numpy[i], level=0
            )
        ...
        try:
            verts, faces, normals, values = measure.marching_cubes(
                psr_grid_numpy, level=zero_level
            )
        except:
...
def mesh_rasterization(verts, faces, pose, img_size):
    ...
    """
    transformed_v = pose.transform_points(
        verts.detach()
    )  # world -> ndc coordinate system
...
def load_pointcloud(in_file):
    ...
    vertices = np.stack(
        [plydata["vertex"]["x"], plydata["vertex"]["y"], plydata["vertex"]["z"]], axis=1
    )
    return vertices
...
def initialize_logger(cfg):
    ...
    cfg["training"]["dir_model"] = os.path.join(out_dir, "model")
    os.makedirs(cfg["training"]["dir_model"], exist_ok=True)

    if cfg["training"]["exp_mesh"]:
        cfg["training"]["dir_mesh"] = os.path.join(out_dir, "vis/mesh")
        os.makedirs(cfg["training"]["dir_mesh"], exist_ok=True)
    if cfg["training"]["exp_pcl"]:
        cfg["training"]["dir_pcl"] = os.path.join(out_dir, "vis/pointcloud")
        os.makedirs(cfg["training"]["dir_pcl"], exist_ok=True)
    if cfg["training"]["vis_rendering"]:
        cfg["training"]["dir_rendering"] = os.path.join(out_dir, "vis/rendering")
        os.makedirs(cfg["training"]["dir_rendering"], exist_ok=True)
    if cfg["training"]["o3d_show"]:
        cfg["training"]["dir_o3d"] = os.path.join(out_dir, "vis/o3d")
        os.makedirs(cfg["training"]["dir_o3d"], exist_ok=True)
    ...
    fh = logging.FileHandler(os.path.join(cfg["training"]["out_dir"], "log.txt"))
...
def update_optimizer(inputs, cfg, epoch, model=None, schedule=None):
    if model is not None:
        ...
        elif "lr" in cfg["training"]:
            optimizer = torch.optim.Adam(
                [
                    {"params": model.parameters(), "lr": float(cfg["training"]["lr"])},
                    {"params": inputs, "lr": float(cfg["training"]["lr_pcl"])},
                ]
            )
        else:
        ...
        if schedule is not None:
            optimizer = torch.optim.Adam(
                [inputs], lr=schedule[0].get_learning_rate(epoch)
            )
        else:
            optimizer = torch.optim.Adam([inputs], lr=float(cfg["training"]["lr_pcl"]))
...
class StepLearningRateSchedule(LearningRateSchedule):
    ...
    def get_learning_rate(self, epoch):
        lr = np.maximum(
            self.initial * (self.factor ** (epoch // self.interval)), 5.0e-6
        )
        if lr > self.final:
            return lr
        else:
