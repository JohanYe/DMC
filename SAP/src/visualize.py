...
def visualize_points_mesh(
    vis, points, normals, verts, faces, cfg, it, epoch, color_v=None
):
    """Visualization.
...
def visualize_psr_grid(psr_grid, pose=None, out_dir=None, out_video_name="video.mp4"):
    if pose is not None:
        ...
        co_grid = (
            torch.stack([co_x.reshape(-1), co_y.reshape(-1), co_z.reshape(-1)], dim=1)
            .to(device)
            .unsqueeze(0)
        )
        ...
        in_mask = psr_grid < 0
        pix = (
            pose.transform_points_screen(co_grid, ((res, res),))[..., :2]
            .round()
            .long()
            .squeeze()
        )
        ...
        for idx in trange(s):
            ...
            plt.subplot(1, 3, 1)
            plt.imshow(
                ndimage.rotate(psr_grid[idx], 180, mode="nearest"), cmap="nipy_spectral"
            )
            plt.clim(-1, 1)
            plt.colorbar()
            plt.title("x")
            ...
            plt.subplot(1, 3, 2)
            plt.imshow(
                ndimage.rotate(psr_grid[:, idx], 180, mode="nearest"),
                cmap="nipy_spectral",
            )
            plt.clim(-1, 1)
            plt.colorbar()
            ...
            plt.subplot(1, 3, 3)
            plt.imshow(
                ndimage.rotate(psr_grid[:, :, idx], 90, mode="nearest"),
                cmap="nipy_spectral",
            )
            plt.clim(-1, 1)
            plt.colorbar()
            ...
            plt.savefig(
                os.path.join(dir_psr_vis, "{}".format(idx)), pad_inches=0, dpi=100
            )
            plt.close()
        os.system("rm {}/{}".format(dir_psr_vis, out_video_name))
        os.system(
            "ffmpeg -framerate 25 -start_number 0 -i {}/%d.png -pix_fmt yuv420p -crf 17 {}/{}".format(
                dir_psr_vis, dir_psr_vis, out_video_name
            )
        )
...
def visualize_mesh_phong(v, f, n, pose, img_size, name, device="cpu"):
    ...
    diffuse = torch.mm(n_inters, light).clamp_min(0).repeat(
        1, 3
    ) * diffuse_per.unsqueeze(0).to(device)
...
def render_rgb(
    v, f, n, pose, renderer, img_size, mask_gt=None, ray=None, fea_grid=None
):
    p_inters, mask, f_p, w = calc_inters_points(
        v.detach(), f, pose, img_size, mask_gt=mask_gt
    )
    ...
    color_pred = renderer(
        p_inters, normals=n_inters, view_dirs=ray, feature_vectors=fea
    ).squeeze()
