...
class Trainer(object):
    ...
    def __init__(self, cfg, optimizer, device=None):
        ...
        if cfg["training"]["gauss_weight"] > 0.0:
    ...
    def train_step(self, inputs, data, model):
        ...
        if self.cfg["training"]["w_psr"] != 0:
            ...
            if self.cfg["training"]["gauss_weight"] > 0:
                gauss_sigma = self.cfg["training"]["gauss_weight"]
                # set up the weighting for loss, higher weights
                # for points near to the surface
                psr_gt_pad = torch.nn.ReplicationPad3d(1)(psr_gt.unsqueeze(1)).squeeze(
                    1
                )
                delta_x = delta_y = delta_z = 1
                grad_x = (
                    (psr_gt_pad[:, 2:, :, :] - psr_gt_pad[:, :-2, :, :]) / 2 / delta_x
                )
                grad_y = (
                    (psr_gt_pad[:, :, 2:, :] - psr_gt_pad[:, :, :-2, :]) / 2 / delta_y
                )
                grad_z = (
                    (psr_gt_pad[:, :, :, 2:] - psr_gt_pad[:, :, :, :-2]) / 2 / delta_z
                )
                ...
                loss_each["psr"] = self.cfg["training"]["w_psr"] * F.mse_loss(
                    w * psr_grid, w * psr_gt
                )
            else:
                loss_each["psr"] = self.cfg["training"]["w_psr"] * F.mse_loss(
        ...
        if self.cfg["training"]["w_reg_point"] != 0.0:
            points_gt = data.get("gt_points").to(self.device)
            loss_reg, loss_norm = chamfer_distance(points, points_gt)

            loss_each["reg"] = self.cfg["training"]["w_reg_point"] * loss_reg
            loss += loss_each["reg"]

        if self.cfg["training"]["w_normals"] != 0.0:
            points_gt = data.get("gt_points").to(self.device)
            ...
            loss_each["normals"] = self.cfg["training"]["w_normals"] * loss_norm
    ...
    def save(self, model, data, epoch, id):
        ...
        if exp_pcl:
            ...
            export_pointcloud(
                os.path.join(dir_pcl, "{:04d}_{:01d}.ply".format(epoch, id)),
                scale2onet(points),
                normals,
            )
            if exp_gt:
                export_pointcloud(
                    os.path.join(dir_pcl, "{:04d}_{:01d}_oracle.ply".format(epoch, id)),
                    scale2onet(points_gt),
                    normals_gt,
                )
            if exp_input:
                export_pointcloud(
                    os.path.join(dir_pcl, "{:04d}_{:01d}_input.ply".format(epoch, id)),
                    scale2onet(p),
                )
        ...
        if exp_mesh:
            ...
            with torch.no_grad():
                v, f, _ = mc_from_psr(
                    psr_grid, zero_level=self.cfg["data"]["zero_level"]
                )
            ...
            if exp_gt:
                ...
                with torch.no_grad():
                    v, f, _ = mc_from_psr(
                        psr_gt, zero_level=self.cfg["data"]["zero_level"]
                    )
                export_mesh(
                    os.path.join(
                        dir_mesh, "{:04d}_{:01d}_oracle.ply".format(epoch, id)
                    ),
                    scale2onet(v),
                    f,
                )
